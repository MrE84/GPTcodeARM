//https://srituhobby.com/how-to-make-a-bluetooth-controlled-robot-arm-with-arduino/
...
//SoftwareSerial getValue(2,3); // TX , RX
...
pwm.setPWM(2,0,pulseWidth(120)); //WAAAIST_SERVO
pwm.setPWM(3,0,pulseWidth(97)); //SHOULDER_SERVO
pwm.setPWM(4,0,pulseWidth(88)); //ELBOW_SERVO 
...
void loop() {
  ...
  if (bt.available()) { // Check if data is available on the serial port
...
//   if (getValue.available() > 0) { // Check if data is available on the serial port
//    String command = getValue.readStringUntil('\n'); // Read the command
//    executeCommand(command);
//  }
